...
def init_gui(_map_size):
    ...
    grid_surf = draw_grid(map_size, border, square_size)
...
def _refresh_menu(window_size, font_size):
    ...
    stddraw.picture(flower_image, 1.5 * size/2, y)
    stddraw.picture(hive_image, 4.6*size/2, y)
    stddraw.picture(desert_hive_image, 8.25*size/2, y)
    stddraw.picture(honey_hive_image, 13.1*size/2, y)
    stddraw.picture(wasp_hive_image, 17.75*size/2, y)
    ...
    stddraw.picture(bee_image, 2 * size/2, y)
    stddraw.picture(desert_bee_image, 5.75*size/2, y)
    stddraw.picture(forager_image, 10.4*size/2, y)
    stddraw.picture(scout_image, 14.3*size/2, y)
    stddraw.picture(wasp_image, 17.6*size/2, y)    
...
def render_frame(map_board, begin):
    global frame_show_ms
    global window_size
    global map_size
    global grid_surf
    ...
    stddraw._surface.blit(grid_surf, (0,0))
    ...
    for row in range(map_size):
        for col in range(map_size):
            ...
            y = border + ((map_size-row -1) * square_size) + square_size/2
...
def draw_grid(map_size, border, square_size):
    ...
    white = pygame.Color("white")
    green = stddraw._pygameColor(stddraw.GREEN)
    dark_green = stddraw._pygameColor(stddraw.DARK_GREEN)
    ...
    for row in range(map_size):
        ...
        for col in range(map_size):
            x = border + (col * square_size) + square_size/2
            y = window_size - (border + (row * square_size) + square_size/2)
            c = green if (row + col) % 2 == 0 else dark_green

            rect = pygame.Rect(x - square_size/2, y - square_size/2, square_size, square_size)
            pygame.draw.rect(surf, c, rect)
    
    pygame.draw.line(surf, pygame.Color("black"),(border, 0), (border, window_size-border), int(square_size/14))
    pygame.draw.line(surf, pygame.Color("black"),(border, window_size-border), (window_size, window_size-border), int(square_size/14))
    return surf
